...
function setCookies(cname,cvalue,exdays) {
    if(getCookie("campaignCookie") != ""){
        cvalue = getCookie("campaignCookie") + "," + cvalue
    }
    const d = new Date();
...
Swal.fire({
    ...
    preConfirm: function() {
        return new Promise(function(a, e) {
            //for visitor with one email
            if($("#visitorEmail").val() != "" && visitor == true){
                groups = [];
                groups.push({
    ...
    e.value && Swal.fire("Campaign Scheduled!", "This campaign has been scheduled for launch!", "success"), $('button:contains("OK")').on("click", function() {
        if(visitor){
            setCookies("campaignCookie",campaign.id.toString(),1)
        }
...
api.groups.summary().success(function(e) {
    if (groups = e.groups, 0 == groups.length) return modalError("No recipient found!"), !1;
    var a = $.map(groups, function(e) {
        return e.text = e.name, e.title = e.num_targets + " targets", e
    });
    u = $("#users.form-control");
    u.select2({
        placeholder: "Select Recipient",
        data: a
    }), 1 === a.length && (u.val(a[0].id), u.trigger("change.select2"))
    ...
    var a = $.map(e, function(e) {
        ...
        t = $("#page.form-control");
    t.select2({
        placeholder: "Select a Capture Page",
        data: a
    }), 1 === e.length && (t.val(a[0].id), t.trigger("change.select2"))
...
$("#launch_date").datetimepicker({
    ...
    campaigns = e.campaigns, $("#loading").hide(), 0 < campaigns.length ? ($("#campaignTable").show(), $("#campaignTableArchive").show(), activeCampaignsTable = $("#campaignTable").DataTable({
        ...
        if(visitor){
            var n = [escapeHtml(a.name), moment(a.created_date).format("MMMM Do YYYY, h:mm:ss a"), '<span class="label ' + label + '" data-toggle="tooltip" data-placement="right" data-html="true" title="' + t + '">' + a.status + "</span>", 
            "<div class='pull-right'><a class='btn btn-primary' onclick='passwordProtect("
            + a.id + 
            ")' data-toggle='tooltip' data-placement='left' title='View Results'><i class='fa fa-bar-chart'></i></a><span data-toggle='modal' data-backdrop='static' data-target='#modal'></span> "
             ];
        }
        else{
            var n = [escapeHtml(a.name), moment(a.created_date).format("MMMM Do YYYY, h:mm:ss a"), '<span class="label ' + label + '" data-toggle="tooltip" data-placement="right" data-html="true" title="' + t + '">' + a.status + "</span>", 
