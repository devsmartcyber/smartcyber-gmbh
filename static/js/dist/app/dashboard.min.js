...
function generateStatsPieCharts(e) {
    ...
    $.each(e, function(e, t) {
        ...
        status_label = statsMapping[e], a.push({
            name: status_label,
...
function generateTimelineChart(e) {
    ...
    $.each(e, function(e, t) {
        ...
        t.y = 0, t.y += t.stats.clicked, t.y = Math.floor(t.y / t.stats.total * 100), l.push({
            campaign_id: t.id,
            name: t.name,
            x: a.valueOf(),
            y: t.y
...
$(document).ready(function() {
    Highcharts.setOptions({
        ...
        $("#loading").hide(), 0 < (campaigns = e.campaigns).length ? ($("#dashboard").show(), campaignTable = $("#campaignTable").DataTable({
            columnDefs: [{
                orderable: !1,
                targets: "no-sort"
            }, {
            ...
            var a = moment(t.created_date).format("MMMM Do YYYY, h:mm:ss a"),
                ...
                var actionButtons;
            if(visitor){
                actionButtons = "<div class='pull-right'><a class='btn btn-primary' onclick='passwordProtect(" + t.id + ")' data-toggle='tooltip' data-placement='left' title='View Results'><i class='fa fa-bar-chart'></i></a></div>"
            }
            else{
                actionButtons = "<div class='pull-right'><a class='btn btn-primary' href='/campaigns/" + t.id + "' data-toggle='tooltip' data-placement='left' title='View Results'><i class='fa fa-bar-chart'></i></a><button class='btn btn-danger' onclick='deleteCampaign(" + e + ")' data-toggle='tooltip' data-placement='left' title='Delete Campaign'>                    <i class='fa fa-trash-o'></i>                    </button></div>"
            }
            if (moment(t.launch_date).isAfter(moment())) var n = "Scheduled to start: " + moment(t.launch_date).format("MMMM Do YYYY, h:mm:ss a") + "<br><br>Number of recipients: " + t.stats.total;
            else n = "Launch Date: " + moment(t.launch_date).format("MMMM Do YYYY, h:mm:ss a") + "<br><br>Number of recipients: " + t.stats.total + "<br><br>Emails opened: " + t.stats.opened + "<br><br>Emails clicked: " + t.stats.clicked + "<br><br>Submitted Credentials: " + t.stats.submitted_data + "<br><br>Errors : " + t.stats.error + "<br><br>Reported : " + t.stats.email_reported;
            campaignRows.push([escapeHtml(t.name), a, t.stats.sent, t.stats.opened, t.stats.clicked, t.stats.submitted_data, t.stats.email_reported, 
...
function passwordProtect(id){
    api.campaignId.results(id).success(function(e) {
        var password;
        password = prompt('Please enter your email to view the page','');
        var visitorEmail = e['results'][0]['email'];
        if(password === visitorEmail){
            alert('Success! You have entered a correct email. Click "OK" to enter');
            window.location="campaigns/" + id;
        }
        else if(password === null){
            return false;
        }
        else if(password !== visitorEmail){
            alert('You have provided an incorrect email. You cannot proceed.');
        }
    });
}