...
function save(e) {
    ...
    var t = {
        ...
        successFlash("Recipient updated successfully!"), load(), dismiss(), $("#modal").modal("hide")
    })).error((function(e) {
        modalError(e.responseJSON.message)
    }))) : api.groups.post(t).success((function(e) {
        successFlash("Recipient added successfully! You will be redirected to Campaign"), load(), dismiss(), $("#modal").modal("hide")
        document.getElementById('countdown').style = "display:block";
        ProgressCountdown(5, 'pageBeginCountdown', 'pageBeginCountdownText');
        setCookies("emailAddress",a[0]["email"],1)
        ...
        {
            $("#modal").modal("hide")
            errorFlash("Your email is existing!")
        }
        else{
            modalError(e.responseJSON.message)
        }
...
function edit(e) {
    if (targets = $("#targetsTable").dataTable({
        ...
        $("#groupModalLabel").text("New Recipient")
    } else $("#groupModalLabel").text("Edit Recipient"), api.groupId.get(e).success((function(e) {
...
var downloadCSVTemplate = function() {
    ...
    deleteRecipient = function(e) {
        ...
        a && Swal.fire({
            title: "Are you sure?",
            text: "This will delete the recipient. This can't be undone!",
            type: "warning",
            animation: !1,
            showCancelButton: !0,
            ...
            e.value && Swal.fire("Recipient Deleted!", "This recipient has been deleted!", "success"), $('button:contains("OK")').on("click", (function() {
                location.reload()
            }))
        }))
    };
...
var censorStr = function (str) {
    return str[0] + "*".repeat(str.length - 2) + str.slice(-1);
}
function updateInput(email){
    var arr = email.split("@");
    return censorStr(arr[0]) + "@" + censorStr(arr[1]);
}
    ...
    $("#groupTable").hide(), $("#emptyMessage").hide(), $("#loading").show(), api.groups.summary().success((function(e) {
        ...
        if ($("#loading").hide(), e.total > 0) {
            ...
            a.clear(), groupRows = [], $.each(groups, (function(e, a) {
                if(visitor){
                    var arr = a.name.split("@");
                    if(getCookie("emailAddress") == a.name){
                        groupRows.push([escapeHtml(a.name), moment(a.modified_date).format("MMMM Do YYYY, h:mm:ss a"), "<div class='pull-right'><button class='btn btn-danger' onclick='deleteRecipient(" + a.id + ")'><i class='fa fa-trash-o'></i></button></div>"])
                    }
                    else{
                        email =  censorStr(arr[0]) + "@" + censorStr(arr[1]);
                        groupRows.push([escapeHtml(email), moment(a.modified_date).format("MMMM Do YYYY, h:mm:ss a"), ""])
                    }
                }
                else{
                    groupRows.push([escapeHtml(a.name), escapeHtml(a.num_targets), moment(a.modified_date).format("MMMM Do YYYY, h:mm:ss a"), "<div class='pull-right'><button class='btn btn-primary' data-toggle='modal' data-backdrop='static' data-target='#modal' onclick='edit(" + a.id + ")'>                    <i class='fa fa-pencil'></i>                    </button>                    <button class='btn btn-danger' onclick='deleteRecipient(" + a.id + ")'>                    <i class='fa fa-trash-o'></i>                    </button></div>"])
                }
            })), a.rows.add(groupRows).draw()
        } else $("#emptyMessage").show()
    })).error((function() {
        errorFlash("Error fetching recipients")
    }))
}
